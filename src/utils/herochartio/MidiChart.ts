                ...
                if (elt.type == "setTempo")
                    ...
                    if (trackName == "EVENTS") {
                        ...
                        if (sectionName && sectionName[1])
                            chart.pushTrackData(chart.Events, time, {
                                type: "E",
                                name: sectionName[1],
                            })
                        ...
                        for (let i = 0; i < difficulties.length; i++) {
                            ...
                            if (
                                ...
                                for (let i = index + 1; i < obj_track.length; i++) {
                                    ...
                                    if (
                                        offelt.noteNumber == elt.noteNumber &&
                                        offelt.type == "noteOff"
                                    )
                                        break
                                }
                                ...
                                if (
                                    duration <= chart.Song.Resolution / 3 ||
                                    elt.noteNumber == octaveNote + 5
                                )
                                    duration = 0
        ...
        for (const chartTrackName in chart.tracks) {
            for (const t in chart.tracks[chartTrackName]) {
                if (chart.tracks[chartTrackName][t].length > 1) {
                    chart.tracks[chartTrackName][t] = chart.tracks[chartTrackName][t].sort(
                        (a, b) => {
